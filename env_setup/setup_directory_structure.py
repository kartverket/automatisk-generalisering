...
def create_gdbs_in_subdirs(base_directory, sub_directories, gdb_names):
    for subdir in sub_directories:
        subdir_path = os.path.join(base_directory, 'automatic_generalization_outputs', subdir)
        for gdb_name in gdb_names:
            gdb_path = os.path.join(subdir_path, f"{gdb_name}.gdb")
            if not arcpy.Exists(gdb_path):
                arcpy.CreateFileGDB_management(out_folder_path=subdir_path, out_name=f"{gdb_name}.gdb")
