...
def create_append_feature_class():
    ...
    input_building_points = f"{Building_N100.calculate_point_values___points_pre_resolve_building_conflicts___n100_building.value}_copy"
...
def move_edge_objects(partition_polygon):
    arcpy.management.Copy(
        in_data=Building_N100.calculate_point_values___points_pre_resolve_building_conflicts___n100_building.value,
        out_data=f"{Building_N100.calculate_point_values___points_pre_resolve_building_conflicts___n100_building.value}_copy",
    )

    arcpy.management.SelectLayerByLocation(
        in_layer=f"{Building_N100.calculate_point_values___points_pre_resolve_building_conflicts___n100_building.value}_copy",
        overlap_type=custom_arcpy.OverlapType.BOUNDARY_TOUCHES,
    )

    custom_arcpy.select_location_and_make_feature_layer(
        input_layer=f"{Building_N100.calculate_point_values___points_pre_resolve_building_conflicts___n100_building.value}_copy",
        overlap_type=custom_arcpy.OverlapType.BOUNDARY_TOUCHES,
        select_features=partition_polygon,
        output_name=f"{Building_N100.calculate_point_values___points_pre_resolve_building_conflicts___n100_building.value}_copy",
    )
