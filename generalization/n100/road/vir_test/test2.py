...
def get_kommune_names():
    """Retrieve unique kommune names from the input_n50.AdminFlate layer."""
    kommune_names = set()
    with arcpy.da.SearchCursor(input_n50.AdminFlate, ["NAVN"]) as cursor:
        for row in cursor:
            kommune_names.add(row[0])
    return list(kommune_names)


@timing_decorator
def main():
    environment_setup.main()
    kommune_names = get_kommune_names()

    for kommune_name in kommune_names:
        print(f"Processing kommune: {kommune_name}")
...
def kommune(kommune_name):
    """Select a kommune by NAVN and create a permanent feature."""
    custom_arcpy.select_attribute_and_make_permanent_feature(
        input_layer=input_n50.AdminFlate,
        expression=f"NAVN='{kommune_name}'",
...
def veg100_kommune_name(kommune_name):
    """Clip the final output with the selected kommune and save with a dynamic name."""
    # Create the output name using kommune_name directly
    output_name = f"veg100_{kommune_name}_n100_road"
    arcpy.analysis.Clip(
        in_features=Road_N100.test1___elveg_and_sti_kommune_singlepart_dissolve___n100_road.value,
...
#         clip_features=Road_N100.test1___kommune___n100_road.value,
#         out_feature_class=Road_N100.test1___veg100_vadsø2___n100_road.value,
#     )
