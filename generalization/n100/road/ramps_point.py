...
def combine_intersecting_buffers(buffer_fc, out_fc, max_per_cluster = 4):
    ...
    with arcpy.da.InsertCursor(out_fc_final, ["SHAPE@", "ClusterID", "NumberOfBuffer"]) as icur:
...
def create_near_map(distance_str, in_fc, near_fc,):
    ...
    near_map = {}
    with arcpy.da.SearchCursor(near_table, ["IN_FID", "NEAR_X", "NEAR_Y", "NEAR_DIST", "NEAR_RANK", "NEAR_FID"]) as s:
...
def make_ramp_points() -> None:
    ...
    arcpy.management.MakeFeatureLayer(
        output_fc, "roads_lyr", where_clause="typeveg <> 'rampe' and objtype = 'Veglenke'"
    )
    arcpy.management.MakeFeatureLayer(
...
def connect_roads_to_points():
    ...
    arcpy.management.MakeFeatureLayer(ramp_points_fc, "ramp_points_34_lyr", where_clause="priority = 3 or priority = 3.5 or priority = 4")


    arcpy.management.SelectLayerByLocation("roads_lyr", "INTERSECT", "ramps_lyr", selection_type="NEW_SELECTION")
    ...
    with arcpy.da.SearchCursor("roads_lyr", ["OID@",  "SHAPE@"]) as road_cur, \
        arcpy.da.InsertCursor(endpoints_fc, ["SHAPE@", "from_road", "start_end"]) as ins_cur:
        for oid, geom in road_cur:
    ...
    arcpy.management.SelectLayerByLocation("endpoints_lyr", "INTERSECT", "ramps_lyr", selection_type="NEW_SELECTION")
    ...
    near_table = "in_memory\\near_table"
    arcpy.GenerateNearTable_analysis(in_features="endpoints_lyr",
    ...
    near_map = {}
    with arcpy.da.SearchCursor(near_table, ["IN_FID", "NEAR_FID", "NEAR_X", "NEAR_Y", "NEAR_DIST", "NEAR_RANK"]) as s:
    ...
    with arcpy.da.SearchCursor("endpoints_lyr", ["OID@", "from_road", "start_end"]) as end_cur:
        end_dict = {}
...
class MovePointsToCrossings:
    ...
    def make_priority_points(self):
        ...
        self.delete_list = [buffer_500m, buffer_100m, buffer_100m_dissolved, roads_lyr, motorveg_t_lyr, motorveg_l_lyr, motorveg_u_lyr, ikke_motorveg_t_lyr, ikke_motorveg_l_lyr, ikke_motorveg_u_lyr, roads_t_lyr, roads_l_lyr, roads_u_lyr, roads_ul_lyr, priority1_lyr, priority1_5_lyr, priority2_lyr]

        
        arcpy.management.MakeFeatureLayer(self.input_road_feature, roads_lyr, where_clause="typeveg <> 'rampe' and objtype = 'Veglenke'")
        # Select roads within 500 meters to iterate over fewer objects
        arcpy.analysis.Buffer(self.input_point_feature, buffer_500m, "500 Meters", dissolve_option="ALL")
        arcpy.management.SelectLayerByLocation(roads_lyr, "INTERSECT", buffer_500m, selection_type="NEW_SELECTION")
        ...
        self.make_priority1_points(motorveg_t_lyr, motorveg_l_lyr, motorveg_u_lyr, roads_ul_lyr, self.priority1)
        self.make_priority1_5_points(motorveg_t_lyr, motorveg_l_lyr, motorveg_u_lyr, ikke_motorveg_t_lyr, ikke_motorveg_l_lyr, ikke_motorveg_u_lyr, roads_ul_lyr, self.priority1_5)
        self.make_priority2_points(roads_t_lyr, roads_l_lyr, roads_u_lyr, roads_ul_lyr, self.priority2)
        ...
        if self.with_ramps:
            arcpy.management.MakeFeatureLayer(self.input_road_feature, ramps_lyr, where_clause="typeveg = 'rampe'")
            arcpy.analysis.Buffer(ramps_lyr, buffer_100m, "100 Meters")
            combine_intersecting_buffers(buffer_100m, "buffer_ramps_100m_dissolved", max_per_cluster=1000)

            
            arcpy.management.MakeFeatureLayer(self.priority1, priority1_lyr)
            arcpy.management.SelectLayerByLocation(priority1_lyr, "INTERSECT", buffer_100m_dissolved, selection_type="NEW_SELECTION")
            arcpy.management.SelectLayerByAttribute(priority1_lyr, "SWITCH_SELECTION", "1=1")
            ...
            arcpy.management.SelectLayerByLocation(priority1_5_lyr, "INTERSECT", buffer_100m_dissolved, selection_type="NEW_SELECTION")
            arcpy.management.SelectLayerByAttribute(priority1_5_lyr, "SWITCH_SELECTION", "1=1")
            ...
            arcpy.management.SelectLayerByLocation(priority2_lyr, "INTERSECT", buffer_100m_dissolved, selection_type="NEW_SELECTION")
            arcpy.management.SelectLayerByAttribute(priority2_lyr, "SWITCH_SELECTION", "1=1")
    ...
    def make_priority_maps(self):
        ...
        if self.with_ramps:
            self.near1_map = self.create_near_map_unmatched_buffer("500 Meters", self.input_point_feature, self.priority1, all_oids, "in_memory\\buffer_ramps_100m_dissolved")
        else:
            self.near1_map = self.create_near_map_unmatched("200 Meters", self.input_point_feature, self.priority1, all_oids)
        self.unmatched_oids = [oid for oid in all_oids if oid not in self.near1_map]


        if self.with_ramps:
            self.near1_5_map = self.create_near_map_unmatched_buffer("500 Meters", self.input_point_feature, self.priority1_5, self.unmatched_oids, "in_memory\\buffer_ramps_100m_dissolved")   
        else:
            self.near1_5_map = self.create_near_map_unmatched("200 Meters", self.input_point_feature, self.priority1_5, self.unmatched_oids)  
        self.unmatched_oids = [oid for oid in self.unmatched_oids if oid not in self.near1_5_map]

        if self.with_ramps:
            self.near2_map = self.create_near_map_unmatched_buffer("500 Meters", self.input_point_feature, self.priority2, self.unmatched_oids, "in_memory\\buffer_ramps_100m_dissolved")  
        else:
            self.near2_map = self.create_near_map_unmatched("200 Meters", self.input_point_feature, self.priority2, self.unmatched_oids)
        self.unmatched_oids = [oid for oid in self.unmatched_oids if oid not in self.near2_map]

        if not self.with_ramps:
            self.near2_5_map = self.create_near_map_unmatched("200 Meters", self.input_point_feature, self.piority2_5, self.unmatched_oids)
            self.unmatched_oids = [oid for oid in self.unmatched_oids if oid not in self.near2_5_map]
        ...
        if self.with_ramps:
            self.near3_map = self.create_near_map_unmatched_buffer("100 Meters", self.input_point_feature, "motorveg_lyr", self.unmatched_oids, "in_memory\\buffer_ramps_100m_dissolved")
        else:
            self.near3_map = self.create_near_map_unmatched("100 Meters", self.input_point_feature, "motorveg_lyr", self.unmatched_oids)
        self.unmatched_oids = [oid for oid in self.unmatched_oids if oid not in self.near3_map]


        arcpy.management.MakeFeatureLayer(roads_lyr, "ikke_motorveg_lyr", where_clause="motorvegtype = 'Ikke motorveg'")
        if self.with_ramps:
            self.near3_5_map = self.create_near_map_unmatched_buffer("100 Meters", self.input_point_feature, "ikke_motorveg_lyr", self.unmatched_oids, "in_memory\\buffer_ramps_100m_dissolved")
        else:
            self.near3_5_map = self.create_near_map_unmatched("100 Meters", self.input_point_feature, "ikke_motorveg_lyr", self.unmatched_oids)
        self.unmatched_oids = [oid for oid in self.unmatched_oids if oid not in self.near3_5_map]

        if self.with_ramps:
            self.near4_map = self.create_near_map_unmatched_buffer("100 Meters", self.input_point_feature, "roads_lyr", self.unmatched_oids, "in_memory\\buffer_ramps_100m_dissolved")
        else:
            self.near4_map = self.create_near_map_unmatched("100 Meters", self.input_point_feature, "roads_lyr", self.unmatched_oids)
    ...
    def place_points(self):
        ...
        existing_out_fields = [f.name for f in arcpy.ListFields(self.output_point_feature) if f.type not in ("OID", "Geometry")]
        out_fields = ["SHAPE@", "priority", "roadID"] + existing_out_fields
        in_fields = [arcpy.Describe(self.input_point_feature).oidFieldName, "SHAPE@"] + existing_out_fields
        ...
        with arcpy.da.SearchCursor(self.input_point_feature, in_fields) as scur, arcpy.da.InsertCursor(self.output_point_feature, out_fields) as icur:
            for row in scur:
                oid = int(row[0])
                orig_geom = row[1]
                changed = False
        ...
        # Remove duplicates 
        arcpy.DeleteIdentical_management(in_dataset=self.output_point_feature, fields="Shape", xy_tolerance="100 Meters")



    
    @staticmethod
    def make_priority1_points(motorveg_t_lyr, motorveg_l_lyr, motorveg_u_lyr, roads_ul_lyr, priority1):
        """
        ...
        arcpy.Intersect_analysis([motorveg_t_lyr, motorveg_l_lyr], intersect, join_attributes="ALL", output_type="POINT")
        arcpy.Intersect_analysis([motorveg_t_lyr, motorveg_u_lyr], intersect2, join_attributes="ALL", output_type="POINT")
        arcpy.Intersect_analysis([motorveg_u_lyr, motorveg_l_lyr], priority1, join_attributes="ALL", output_type="POINT")
    ...
    @staticmethod
    def make_priority1_5_points(motorveg_t_lyr, motorveg_l_lyr, motorveg_u_lyr, ikke_motorveg_t_lyr, ikke_motorveg_l_lyr, ikke_motorveg_u_lyr, roads_ul_lyr, priority1_5):
        """
        ...
        arcpy.Intersect_analysis([motorveg_u_lyr, ikke_motorveg_l_lyr], priority1_5, join_attributes="ALL", output_type="POINT")


        arcpy.management.Append([intersect1, intersect2, intersect3, intersect4, intersect5], priority1_5)
    ...
    @staticmethod
    def make_priority2_points(roads_t_lyr, roads_l_lyr, roads_u_lyr, roads_ul_lyr, priority2):
        """
        ...
        arcpy.Intersect_analysis([roads_t_lyr, roads_l_lyr], intersect1, join_attributes="ALL", output_type="POINT")
        arcpy.Intersect_analysis([roads_t_lyr, roads_u_lyr], intersect2, join_attributes="ALL", output_type="POINT")
    ...
    def make_priority2_5_points(roads_lyr, priority2_5):
        ...
        arcpy.CreateFeatureclass_management("in_memory", "collected_endpoints", "POINT", spatial_reference=sr)
        arcpy.CreateFeatureclass_management(os.path.dirname(priority2_5), os.path.basename(priority2_5), "POINT", spatial_reference=sr)
    ...
    @staticmethod
    def create_near_map_unmatched_buffer(distance_str, in_fc, near_fc, unmatched_oids, buffer_fc):
        """
