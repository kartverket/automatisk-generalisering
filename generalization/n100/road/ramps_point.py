...
def get_selected_oids(layer):
    ...
    with arcpy.da.SearchCursor(layer, [oid_field]) as cur:
        for (oid,) in cur:
            oids.add(int(oid))
    return oids
...
def connect_roads_to_points():
    ...
    with arcpy.da.SearchCursor(roads_lyr, ["OID@"]) as cursor:
        for row in cursor:
            oid = row[0]
...
class MovePointsToCrossings:
    ...
    def make_priority_maps(self):
        ...
        if self.with_ramps:
            self.near2_5_map = self.create_near_map_unmatched_buffer(
                "500 Meters",
                self.input_point_feature,
                self.piority2_5,
                self.unmatched_oids,
