...
def fix_simple(line: arcpy.Polyline, near_lookup: dict, oid: str) -> arcpy.Polyline:
    """
    Creates a new line of the input line, using the start and end points, and creating a
    third middle point that is moved away from the original line.

    Args:
        line (arcpy.Polyline): The road line to be adjusted
        near_lookup (dict): A dictionary with other instances that are close
        oid (str): The object ID of the current road line
    
    Returns:
        arcpy.Polyline: A new adjusted polyline
    """
...
def move_line_away(geom: arcpy.Polyline, near_x: float, near_y: float, distance: int) -> arcpy.Polyline:
    """
    Moves the road line away from the road in conflict.

    Args:
        geom (arcpy.Polyline): Geometry to change
        near_x (float): X coordinate of the closest line
        near_y (float): Y coordinate of the closest line
        distance (int): Distance adjusting the scale used to move the line
    
    Returns:
        arcpy.Polyline: The new geometry with the adjusted polyline
    """
