...
from custom_tools.generalization_tools.road.dissolve_with_intersections import DissolveWithIntersections
...
def creafte_wfm_gdbs(wfm: WorkFileManager) -> dict:
    ...
    other_area_dissolved_fc = wfm.build_file_path(file_name="other_area_dissolved_fc", file_type="gdb")
    ...
    water_outline_fc = wfm.build_file_path(file_name="water_outline_fc", file_type="gdb")
...
def is_narrow(pnt_fc: str, area_fc: str, narrow_field: str) -> None:
    ...
    arcpy.analysis.Near(in_features=pnt_fc, near_features=area_fc, search_radius=50, location="LOCATION", distance_unit="Meters")

    total = int(arcpy.management.GetCount(pnt_fc).getOutput(0))

    with arcpy.da.UpdateCursor(pnt_fc, ["SHAPE@", "NEAR_DIST", "NEAR_X", "NEAR_Y", narrow_field]) as update_cursor:
        for geom, dist, x, y, _ in tqdm(update_cursor, total=total, desc="Updating narrow attribute", colour="yellow", leave=False):
            if dist == -1:
                continue
            ...
            l_env = line.extent
            margin = 0.000001
            l_xmin, l_ymin, l_xmax, l_ymax = l_env.XMin - margin, l_env.YMin - margin, l_env.XMax + margin, l_env.YMax + margin

            best_distance = np.inf
            for area_geom, axmin, aymin, axmax, aymax in area_items:
                if (axmax < l_xmin) or (axmin > l_xmax) or (aymax < l_ymin) or (aymin > l_ymax):
                    continue
...
def remove_road_points_in_water(road_fc: str, output_fc: str, area_selection: str) -> None:
    """
    ...
    """
    # Create WorkFileManager and relevant paths
    ...
    # Select relevant waterbodies and other areas
    ...
    # Create small buffer around water bodies
    water_lyr = "water_lyr"
    other_area_lyr = "other_area_lyr"
    arcpy.management.MakeFeatureLayer(in_features=files["water_fc"], out_layer=water_lyr)
    arcpy.management.SelectLayerByLocation(in_layer=water_lyr, overlap_type="INTERSECT", select_features=files["area_fc"], selection_type="NEW_SELECTION")
    arcpy.management.MakeFeatureLayer(in_features=files["other_area_fc"], out_layer=other_area_lyr)
    arcpy.management.SelectLayerByLocation(in_layer=other_area_lyr, overlap_type="INTERSECT", select_features=files["area_fc"], selection_type="NEW_SELECTION")
    ...
    arcpy.management.FeatureToLine(in_features=files["other_area_dissolved_fc"], out_feature_class=files["area_line_fc"])

    for lyr in [water_lyr, other_area_lyr]:
        if arcpy.Exists(lyr):
            arcpy.management.Delete(lyr)

    # Create a collection of points that should not be moved
    road_keep_lyr = "road_keep_lyr"
    points_to_keep = set()
    arcpy.management.MakeFeatureLayer(in_features=files["simplified_fc"], out_layer=road_keep_lyr, where_clause="medium = 'L' OR medium = 'U' OR typeveg = 'bilferje'")
    ...
    # Fetch roads close to waterbodies
    road_lyr = "road_lyr"
    arcpy.management.MakeFeatureLayer(in_features=files["simplified_fc"], out_layer=road_lyr, where_clause="medium = 'T' AND typeveg <> 'bilferje'")
    ...
    # Create new attributes for water or not and narrow or not
    ...
    if in_water_field not in field_names:
        arcpy.management.AddField(in_table=files["point_fc"], field_name=in_water_field, field_type="SHORT")
        field_names.append(in_water_field)
    if narrow_field not in field_names:
        arcpy.management.AddField(in_table=files["point_fc"], field_name=narrow_field, field_type="SHORT")
        field_names.append(narrow_field)
    
    # Setting default values
    arcpy.management.CalculateField(in_table=files["point_fc"], field=in_water_field, expression=0, expression_type="PYTHON3") # 1 = in water, 0 = not in water
    arcpy.management.CalculateField(in_table=files["point_fc"], field=narrow_field, expression=0, expression_type="PYTHON3") # 1 = narrow area, 0 = not narrow area

    # Detect road points in water and change attribute values
    point_lyr = "road_points_lyr"
    arcpy.management.MakeFeatureLayer(in_features=files["point_fc"], out_layer=point_lyr)
    arcpy.management.SelectLayerByLocation(
        in_layer=point_lyr,
        overlap_type="INTERSECT",
        select_features=files["water_buffer_fc"],
    ...
    arcpy.management.CalculateField(in_table=point_lyr, field=in_water_field, expression=1, expression_type="PYTHON3")

    # Detect road points in narrow areas and change attribute values
    area_lyr = "area_lyr"
    arcpy.management.MakeFeatureLayer(in_features=files["area_line_fc"], out_layer=area_lyr)

    is_narrow(point_lyr, area_lyr, narrow_field)
    
    # Clear selection and copy to final point layer
    arcpy.management.SelectLayerByAttribute(in_layer_or_view=point_lyr, selection_type="CLEAR_SELECTION")
    arcpy.management.CopyFeatures(
        in_features=point_lyr, out_feature_class=files["point_water_fc"]
    )

    # Collect the different points and the relevant roads in sets
    water_point_lyr = r"water_point_lyr"
    arcpy.management.MakeFeatureLayer(in_features=files["point_water_fc"], out_layer=water_point_lyr)
    arcpy.management.SelectLayerByAttribute(in_layer_or_view=water_point_lyr, selection_type="NEW_SELECTION", where_clause=f"{in_water_field} = 1 AND {narrow_field} = 0")

    arcpy.management.FeatureToLine(in_features=files["water_buffer_fc"], out_feature_class=files["water_outline_fc"])
    
    arcpy.analysis.Near(in_features=water_point_lyr, near_features=files["water_outline_fc"], search_radius=BUFFER_DIST * 1.2, location="LOCATION", angle="NO_ANGLE")

    road_oids = set()
    mapping = {}

    with arcpy.da.SearchCursor(water_point_lyr, ["OID@", "SHAPE@XY", "ORIG_FID", "NEAR_X", "NEAR_Y"]) as search_cursor:
        for oid, xy, orig_oid, nx, ny in search_cursor:
    ...
    arcpy.management.MakeFeatureLayer(in_features=files["simplified_fc"], out_layer=roads_to_edit, where_clause=sql)

    total = int(arcpy.management.GetCount(roads_to_edit).getOutput(0))

    # Update road geometries excluding points in water
    with arcpy.da.UpdateCursor(roads_to_edit, ["OID@", "SHAPE@"]) as update_cursor:
        for oid, geom in tqdm(update_cursor, total=total, desc="Edit road geometries in water", colour="yellow", leave=False):
            parts = []
            for part in tqdm(geom, desc="Editing geometries", colour="yellow", leave=False):
                points = []
                ...
                for pt in part:
                    ...
                    if prev_x is None or (abs(new_pt.X - prev_x) > EPS or abs(new_pt.Y - prev_y) > EPS):
                        points.append(new_pt)
                        prev_x, prev_y = new_pt.X, new_pt.Y
                if len(points) >= 2:
                    parts.append(arcpy.Array(points))
            if parts:
                new_geom = arcpy.Polyline(arcpy.Array(parts), spatial_reference=geom.spatialReference)
    ...
    arcpy.management.CopyFeatures(in_features=files["smooth_fc"], out_feature_class=output_fc)
