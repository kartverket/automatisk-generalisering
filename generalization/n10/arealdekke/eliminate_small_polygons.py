...
class EliminateSmallPolygons:
    ...
    def eliminate_holes(self, input_fc: str, output_fc: str, selection: str, wfm: WorkFileManager):
        """
        ...
        lines = wfm.build_file_path(
            file_name="eliminate_holes_lines", file_type="gdb"
        )
        ...
        input_copy_layer_potential_elims = "eliminate_holes_input_copy_layer_potential_elims"
        singlepart_layer = "eliminate_holes_singlepart_layer"

        arcpy.management.CopyFeatures(in_features=input_fc, out_feature_class=input_copy)
        self.add_fields(input_copy)
        arcpy.management.MakeFeatureLayer(in_features=input_copy, out_layer=input_copy_layer_selection, where_clause=selection)
        arcpy.management.PolygonToLine(in_features=input_copy_layer_selection, out_feature_class=lines, neighbor_option="IGNORE_NEIGHBORS")
        arcpy.management.MultipartToSinglepart(in_features=lines, out_feature_class=singlepart)
       
        sr = arcpy.Describe(lines).spatialReference
        path, name = os.path.split(potential_holes_lines)
        arcpy.management.CreateFeatureclass(out_path=path, out_name=name, geometry_type="POLYLINE", spatial_reference=sr)
        with arcpy.da.SearchCursor(lines, ["SHAPE@"]) as scur, \
        ...
        arcpy.management.FeatureToPolygon(in_features=potential_holes_lines, out_feature_class=potential_holes_polygons)
        ...
        arcpy.management.MakeFeatureLayer(in_features=input_copy, out_layer=input_copy_layer_potential_elims)
        arcpy.management.SelectLayerByAttribute(in_layer_or_view=input_copy_layer_potential_elims, selection_type="NEW_SELECTION", where_clause=where_clause)
        arcpy.management.SelectLayerByLocation(in_layer=input_copy_layer_potential_elims, overlap_type="INTERSECT", select_features=potential_holes_polygons, selection_type="REMOVE_FROM_SELECTION", invert_spatial_relationship="INVERT")

        arcpy.management.Eliminate(in_features=input_copy_layer_potential_elims, out_feature_class=output_fc, selection="LENGTH")
