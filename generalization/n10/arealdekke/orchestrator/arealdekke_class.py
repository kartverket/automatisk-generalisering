...
class Arealdekke:
    ...
    def __init__(
        ...
        1. The file path to the history file must have existed before program starts
        ...
        """
        # Sets up WorkFileManager for temporary land use files
        self.working_fc = (
        ...
        # Load further data and fetch previous temporary files if
        # the process was stopped during generalization last time
        if (
            ...
            # Fetches initialization parameters from YAML
            # => Becomes able to restart from last stop with partly modified files
    ...
    def preprocess(self) -> None:
        ...
        if not self.__preprocessed:
            ...
            # Copy preprocessed data to temporary land use file and update history / status
            arcpy.management.CopyFeatures(
    ...
    def add_categories(self, categories_config_file: Path) -> None:
        ...
        """
        # Must be preprocessed and no categories can be added forehand
    ...
    def get_locked_categories(self) -> None:
        """
        Copies land use geometries of types set to not accessible
        into an own, separate feature class (self.files["locked_fc"]).
        """
    ...
    def get_category(self, category_title: str) -> None:
        ...
        own, separate feature class (self.files["category_fc"]).

        Args:
            category_title (str): Name for current, relevant category
        """
        temp_lyr = "temp_lyr"
    ...
    def get_arealdekke_data(self, input_data: str) -> None:
        """
        Copies the input data into self.files["arealdekke_fc"].
