...
def objtype(files: dict) -> None:
    ...
    arcpy.management.AddField(
        in_table=files["hoydepunkt_n10"],
        field_name="hoydepunkt_type",
        field_type="TEXT",
    )

    with arcpy.da.UpdateCursor(
        files["hoydepunkt_n10"], ["objtype", "hoydepunkt_type"]
    ) as u_cur:
        for row in u_cur:
            row[1] = row[0]
            row[0] = "høydepunkt"
...
def label_hoyde_int(
    ...
    lyrlclass.textSymbol.symbol.symbol.symbolLayers[0].color = {
        "type": "CIMCMYKColor",
    ...
    second_lc.textSymbol.symbol.symbol.symbolLayers[0].color = {
        "type": "CIMCMYKColor",
        "colorSpace": "DeviceCMYK",
        "values": [100, 0, 0, 0, 100],
    }
...
def create_wfm_gdbs(wfm: WorkFileManager) -> dict:
    ...
    forsenkningspunkt_fkb = wfm.build_file_path(
        file_name="forsenkningspunkt_fkb", file_type="gdb"
    )
    terrengpunkt_fkb = wfm.build_file_path(
        file_name="terrengpunkt_fkb", file_type="gdb"
    )
    n50_hoydepunkt_buffers = wfm.build_file_path(
        file_name="n50_hoydepunkt_buffers", file_type="gdb"
    )
    selected_forsenkningspunkt = wfm.build_file_path(
        file_name="selected_forsenkningspunkt", file_type="gdb"
    )
    selected_forsenkningspunkt_buffers = wfm.build_file_path(
        file_name="selected_forsenkningspunkt_buffers", file_type="gdb"
    )
    selected_terrengpunkt = wfm.build_file_path(
        file_name="selected_terrengpunkt", file_type="gdb"
    )
    ...
    ArealdekkeFlate_bebygd = wfm.build_file_path(
        file_name="ArealdekkeFlate_bebygd", file_type="gdb"
    )
...
def fetch_data(files: dict) -> None:
    ...
    arcpy.management.CopyFeatures(
        in_features=input_n10.FKBforsenkningspunkt,
        out_feature_class=files["forsenkningspunkt_fkb"],
    )
    arcpy.management.CopyFeatures(
        in_features=input_n10.FKBterrengpunkt,
        out_feature_class=files["terrengpunkt_fkb"],
    )
    arcpy.management.CopyFeatures(
        in_features=input_n10.ArealdekkeFlate_bebygd,
        out_feature_class=files["ArealdekkeFlate_bebygd"],
    )

    arcpy.management.CopyFeatures(
        in_features=input_n10.ArealdekkeFlate,
        out_feature_class="in_memory\\arealdekkeFlate",
    )
    arcpy.management.MakeFeatureLayer(
        in_features="in_memory\\arealdekkeFlate", out_layer="arealdekkeFlate_lyr"
    )
    arcpy.management.SelectLayerByAttribute(
        in_layer_or_view="arealdekkeFlate_lyr",
        selection_type="NEW_SELECTION",
        where_clause="objtype = 'SnøIsbre'",
    )
    arcpy.management.CopyFeatures(
...
def output_hoydepunkt(files: dict):
    arcpy.management.CopyFeatures(
        in_features=files["hoydepunkt_n10"],
        out_feature_class=Hoydepunkt_N10.hoydepunkt_n10.value,
    )
...
def add_nato_codes(files: dict):
    ...
    arcpy.management.AddField(
        in_table=fc,
        field_name="dgfcd_feature_alpha",
        field_type="TEXT",
    )
    arcpy.management.AddField(
        in_table=fc,
        field_name="dgfcd_feature_531",
        field_type="TEXT",
    )
    arcpy.management.AddField(
        in_table=fc,
...
def select_forsenkningspunkt(files: dict, distance: str) -> None:
    ...
    arcpy.analysis.Buffer(
        in_features=files["hoydepunkt_n50"],
        out_feature_class=files["n50_hoydepunkt_buffers"],
    ...
    arcpy.management.SelectLayerByLocation(
        in_layer=fkb_forsenkningspunkt_lyr,
        overlap_type="INTERSECT",
        select_features=files["n50_hoydepunkt_buffers"],
        selection_type="NEW_SELECTION",
    ...
    near_table = "in_memory\\near_table"
    arcpy.analysis.GenerateNearTable(
        in_features=fkb_forsenkningspunkt_lyr,
        near_features=fkb_forsenkningspunkt_lyr,
        out_table=near_table,
        search_radius=distance,
        closest="ALL",
    )

    keep_oids = select_points_from_neartable(
        near_table=near_table, layer=fkb_forsenkningspunkt_lyr
    )

    arcpy.management.SelectLayerByAttribute(
        in_layer_or_view=fkb_forsenkningspunkt_lyr,
        selection_type="REMOVE_FROM_SELECTION",
        where_clause=f"OBJECTID NOT IN ({', '.join(map(str, keep_oids))})",
    )

    arcpy.management.CopyFeatures(
        in_features=fkb_forsenkningspunkt_lyr,
        out_feature_class=files["selected_forsenkningspunkt"],
    )
...
def select_terrengpunkt(files: dict, distance: str) -> None:
    ...
    """
    arcpy.analysis.Buffer(
        in_features=files["selected_forsenkningspunkt"],
        out_feature_class=files["selected_forsenkningspunkt_buffers"],
        buffer_distance_or_field=distance,
    )
    ...
    arcpy.management.SelectLayerByLocation(
        in_layer=fkb_terrengpunkt_lyr,
        overlap_type="INTERSECT",
        select_features=files["selected_forsenkningspunkt_buffers"],
        selection_type="REMOVE_FROM_SELECTION",
    )

    near_table = "in_memory\\near_table"
    arcpy.analysis.GenerateNearTable(
        in_features=fkb_terrengpunkt_lyr,
        near_features=fkb_terrengpunkt_lyr,
        out_table=near_table,
        search_radius=distance,
        closest="ALL",
    )
    ...
    keep_oids = select_points_from_neartable(
        near_table=near_table, layer=fkb_terrengpunkt_lyr
    )

    arcpy.management.SelectLayerByAttribute(
        in_layer_or_view=fkb_terrengpunkt_lyr,
        selection_type="REMOVE_FROM_SELECTION",
        where_clause=f"OBJECTID NOT IN ({', '.join(map(str, keep_oids))})",
    )
    ...
    arcpy.management.CopyFeatures(
        in_features=fkb_terrengpunkt_lyr,
        out_feature_class=files["selected_terrengpunkt"],
    )
...
def merge_and_add_medium(files: dict) -> None:
    ...
    arcpy.management.Merge(
        inputs=[
            files["selected_forsenkningspunkt"],
            files["selected_terrengpunkt"],
            files["hoydepunkt_n50"],
        ],
    ...
    arcpy.management.SelectLayerByLocation(
        in_layer=hoydepunkt_n10_lyr,
        overlap_type="INTERSECT",
        select_features=files["isbreFlate"],
        selection_type="NEW_SELECTION",
    )
...
def populate_hoyde_int(files: Dict[str, str]) -> None:
    ...
    arcpy.AddMessage(
        f"populate_hoyde_int: updated {count_updated} rows, skipped {count_skipped} rows."
    )
