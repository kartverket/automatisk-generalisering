...
def fetch_data(files: dict)->None:
    ...
    arcpy.management.MakeFeatureLayer(in_features=input_roads.road_vegsperring, out_layer=orig_points_unsnapped_lyr)

    orig_road_lyr="orig_road_lyr"
    arcpy.management.MakeFeatureLayer(in_features=input_roads.road_veglenke, out_layer=orig_road_lyr)

    arcpy.edit.Snap(in_features=orig_points_unsnapped_lyr, snap_environment=[[orig_road_lyr, 'EDGE', '0.05']])

    arcpy.management.CopyFeatures(in_features=orig_points_unsnapped_lyr, out_feature_class=files[fc.roadblocks_preprocessed])
    arcpy.management.CopyFeatures(in_features=orig_road_lyr, out_feature_class=files[fc.non_overlapping_roads])
