...
def create_wfm_gdbs(wfm: WorkFileManager) -> dict:
    ...
    target_lines_adjusted = wfm.build_file_path(
        file_name="target_lines_adjusted", file_type="gdb"
    )
    target_lines_adjusted_single = wfm.build_file_path(
        file_name="target_lines_adjusted_single", file_type="gdb"
    )
    target_lines_fully_adjusted = wfm.build_file_path(
        file_name="target_lines_fully_adjusted", file_type="gdb"
    )

    target_lines_w_bearing = wfm.build_file_path(
        file_name="target_lines_w_bearing", file_type="gdb"
    )
...
def establish_target_area(files: dict) -> None:
    ...
    arcpy.management.MakeFeatureLayer(
        in_features=files[fc.input_point], out_layer=input_point_lyr
    )

    input_line_lyr = "input_lines_lyr"
    arcpy.management.MakeFeatureLayer(
        in_features=files[fc.input_line], out_layer=input_line_lyr
    )
    ...
    arcpy.management.MakeFeatureLayer(
        in_features=files[fc.target_area], out_layer=target_area_lyr
    )

    arcpy.analysis.Clip(
        in_features=input_line_lyr,
        clip_features=target_area_lyr,
        out_feature_class=files[fc.target_lines_adjusted],
    )

    # Make sure the clipped roads are treated seperately
    target_lines_adjusted_lyr = "target_lines_adjusted_lyr"
    arcpy.management.MakeFeatureLayer(
        in_features=files[fc.target_lines_adjusted], out_layer=target_lines_adjusted_lyr
    )

    arcpy.management.MultipartToSinglepart(
        in_features=target_lines_adjusted_lyr,
        out_feature_class=files[fc.target_lines_adjusted_single],
    )
    ...
    arcpy.management.MakeFeatureLayer(
        in_features=files[fc.target_lines_adjusted_single],
        out_layer=target_lines_adjusted_single_lyr,
    )
...
def calculate_road_bearing(files: dict) -> None:
    ...
    arcpy.management.MakeFeatureLayer(
        in_features=files[fc.target_lines_fully_adjusted],
        out_layer=target_lines_fully_adjusted_lyr,
    )

    arcpy.management.AddField(
        target_lines_fully_adjusted_lyr, field_name="LINE_BEARING"
    )
    arcpy.management.CalculateGeometryAttributes(
        in_features=target_lines_fully_adjusted_lyr,
        geometry_property=[["LINE_BEARING", "LINE_BEARING"]],
    )

    arcpy.management.CopyFeatures(
        in_features=target_lines_fully_adjusted_lyr,
        out_feature_class=files[fc.target_lines_w_bearing],
    )
...
def rotate_roadblocks(files: dict, out_feature_class, rotation_difference) -> None:
    ...
    arcpy.management.MakeFeatureLayer(
        in_features=files[fc.target_lines_w_bearing],
        out_layer=target_lines_w_bearing_lyr,
    )
    ...
    with arcpy.da.UpdateCursor(
        ...
        for roadblock in cursor_roadblock:
            ...
            with arcpy.da.SearchCursor(
                ...
                for (
                    intersection_road_n_roadblock
                ) in cursor_intersections_road_n_roadblock:
                    if roadblock[0] == intersection_road_n_roadblock[0]:
                        roadblock[1] = (
                            intersection_road_n_roadblock[1] + rotation_difference
                        )
