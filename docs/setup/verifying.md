...
To test that everything is working as intended we can test a little script. Try running the script called `test_setup.py`, located under `root\docs\setup` [test_setup](test_setup.py).
...
Make sure that you have done the [VsCode Arcpy setup](vs_code_setup.md) or [Pycharm Arcpy setup](pycharm_setup.md) correctly.
