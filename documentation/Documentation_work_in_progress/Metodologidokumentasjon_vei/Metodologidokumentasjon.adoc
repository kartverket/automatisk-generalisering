...
| Multipart to singelpart | For å få den rette oppdelingen av vegsegmenter. Nødvendig for de fleste kartografiske generaliseringsverktøyene som kjøres i ettertid. 
| Hierarki | For differansiering mellom viktighet av veg, må vi definere ett hierarki. For kjørbare veger blir hierarkiet bygd fra Funksjonellvegklasse, beskrevet tidligere i dokumentet. Siden Funksjonellvegklasse kun er for veger fra NVDB, brukes vegkategori for å sette opp hierakriet for alt untatt bilveger.
| Remove small lines | Fjerner alle linjer under 100 m for å fjerne unødvenig smårusk
| Colapse Road detail | Fjerner de fleste rundkjøringer. Problemer vi ikke kan løse i førsteomgang er rundkjøringer med ulik medium, slik som i Sinsenkrysset. Dette er per dags dato ikke løst. Det er også slik at det er 122 objekter (tilsvarer en del færre rundkjøringer), som ikke collasper ved den modellen som kjøres i dag. Det er prøvd en del ulike parametere men gir aldri 100% collapse av alle rundkjrøingene. Ved ulike parametere så er det alltid ulik hvilke rundkjøringer som collapser. Den parameteren som foreligger i dag, gir flest collapse. Det vil i neste versjon jobbes med å utvikle ett eget verktøy for å løse dette. 
| Simplify | Forenkler linjene ved å fjærne undøvendige punkter i linjen.
| Thin Road network: Vei | Beholder vegmønster men fjerner detaljene. 
