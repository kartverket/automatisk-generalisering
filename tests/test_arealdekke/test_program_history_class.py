...
class test_program_history_class(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yml", delete=False) as tmp_perfect:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yml", delete=False) as tmp_no_preprocess:
            yaml.dump(data_non_processed, tmp_no_preprocess)
            tmp_no_preprocess.close()
            self.temp_path_no_preprocess = tmp_no_preprocess.name
            self.temp_file_no_preprocess: History_class = History_class(self.temp_path_no_preprocess)

        #File path not exist
        self.non_existing_path="test_non_exist_file.yml"
        self.temp_file_no_pre_exist: History_class = History_class(self.non_existing_path)
    ...
    def test_restore_arealdekke_attributes(self):
        ...
        self.assertEqual(self.temp_file_perfect.restore_arealdekke_attributes(), goal_history_exists)

        # What if a new file was written?
        goal_history_non_preexisting={}
        self.assertEqual(self.temp_file_no_pre_exist.restore_arealdekke_attributes(), goal_history_non_preexisting)

        # What if no preprocessed operations were completed?
        self.assertEqual(self.temp_file_no_preprocess.restore_arealdekke_attributes(), goal_history_non_preexisting)
    ...
    def test_restore_arealdekke_categories(self):
        ...
        response_no_preprocess=self.temp_file_no_preprocess.restore_arealdekke_categories()
    ...
    def test_update_history_top_lvl(self):
        ...
        self.temp_file_perfect.update_history_top_lvl(new_key_perfect, new_value_perfect)
        self.assertEqual(self.temp_file_perfect.get_history_attribute_top_lvl(new_key_perfect), new_value_perfect)
        ...
        self.temp_file_no_pre_exist.update_history_top_lvl(new_key_non_exist, new_value_non_exist)
        self.assertEqual(self.temp_file_no_pre_exist.get_history_attribute_top_lvl(new_key_non_exist), new_value_non_exist)
    ...
    def test_update_history_cat_lvl(self):
        ...
        self.temp_file_no_pre_exist.update_history_cat_lvl(
            title=title_non_exist, 
            key=new_key_non_exist, 
            value=new_value_non_exist
            )
